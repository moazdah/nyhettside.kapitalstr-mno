...
export async function generateArticleDraftFromRadar(id) {
  ...
  if (!draft.title || !draft.dek || draft.paragraphs.length < 3) {
    return { ok: false, status: 'blocked', reason: 'AI-utkastet var for ufullstendig og ble ikke lagret.' };
  }
  ...
  if (!numeric.pass) {
    return {
      ok: false,
      status: 'blocked',
      reason: `Uverifiserte tall i utkastet: ${numeric.unsupported.join(', ')}`,
      numeric,
    };
  }
  ...
  if (!verified.check.pass || verified.check.unsupportedClaims.length || verified.check.numberIssues.length) {
    return {
      ok: false,
      status: 'blocked',
      reason: 'Faktasjekken fant påstander som ikke er godt nok støttet.',
  ...
  const body = draft.paragraphs.join('\n\n');
  const validationNote = `Automatisk kontroll: alle numeriske tokens støttes av faktapakken; AI-faktasjekk ${verified.check.confidence}/100. Menneskelig redaktør må fortsatt godkjenne før publisering.`;
  ...
  const [article] = await sql`
    ...
    VALUES (
      ${slug}, 'draft', ${draft.title}, ${draft.dek}, ${body}, ${draft.section}, 'Kapitalstrøm',
      ${Number(row.ai_score || 0)}, ${`Utkast fra verifisert faktapakke ${row.fact_version} (${row.confidence}/100).`},
      ${`${MODEL}/${RULESET}`}, true, ${validationNote},
  ...
  await sql`
    ...
    VALUES (
      ...
      'passed', ${verified.check.confidence},
      ${JSON.stringify({ numeric, ai: verified.check })}::jsonb,
  ...
  return {
    ...
    slug: article.slug,
    verificationConfidence: verified.check.confidence,
    title: draft.title,
  };
}
