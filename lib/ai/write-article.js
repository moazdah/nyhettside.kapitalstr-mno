...
async function deepSeekJson(system, user, maxTokens = 3600, temperature = 0.18) {
  ...
  if (!response.ok) {
    throw new Error(`DeepSeek artikkelmotor feilet: ${body?.error?.message || `HTTP ${response.status}`}`);
  }
...
- Ingen markdown, punktlister eller interne redaksjonsnotater.
...
- Hvis et faktum har attribution_needed=true, attribuer det naturlig til source_name, for eksempel "Ifølge E24 ..." eller "Reuters skriver ...".
- Hvis attribution_needed=false, trenger du ikke gjenta kilden i hvert avsnitt.
- Hvis kildegrunnlaget er en offisiell institusjon, kan du naturlig skrive "Bank of Japan opplyser ..." der det passer.
...
export async function generateArticleDraftFromRadar(id) {
  ...
  const [row] = await sql`
    SELECT r.id AS radar_id, r.title AS radar_title, r.source_name, r.url AS discovery_url,
           ...
           fp.market_relevance, fp.can_write, fp.confidence
    FROM radar_items r
  ...
  `;
  if (liveExisting) {
    return { ok: true, status: 'existing_live', articleId: liveExisting.id, slug: liveExisting.slug };
  }
