...
VIKTIG:
- Du får bare tittel, kort sammendrag og kildemetadata. Bruk KUN opplysninger som faktisk står i input. Ikke fyll inn datoer, rentevedtak, kursbevegelser, beløp, sitater eller konsekvenser fra egen kunnskap.
- Hvis input er for tynn til å dokumentere hvorfor saken er viktig, skal score normalt være 55 eller lavere og neste_steg være finn_primarkilde eller overvak.
- Vanlig politikk, valg, personkonflikter, kultur, reise, skole, kjendiser, sport og generell samfunnsdebatt er ikke Kapitalstrøm-saker i seg selv. Politikk kan være relevant når input eksplisitt beskriver en konkret økonomisk beslutning, skatt/budsjett/regulering, rente-/valutaeffekt eller en observert markedsreaksjon.
- Ikke gi høy score fordi noe KAN påvirke markedet. 60+ krever en konkret finans-/økonomikobling i input.
- Kommentarstoff, debatt og generelle spådommer uten nye fakta skal normalt ikke over 55.
- 90–100 krever en eksplisitt, konkret og dokumentert hendelse med åpenbar stor betydning.
...
export async function scorePendingRadarItems(limit = 30) {
  ...
  return { requested: pending.length, scored, model: MODEL_TAG, estimatedCostUsd, usage: result.usage };
}
