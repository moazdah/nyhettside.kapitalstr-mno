...
async function callDeepSeek(items, attempt = 0) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new Error('DeepSeek brukte mer enn 25 sekunder og ble avbrutt trygt.');
    }
    throw error;
  } finally {
    clearTimeout(timeout);
  ...
  if (!response.ok) {
    if ((response.status === 429 || response.status >= 500) && attempt < 1) {
      ...
      return callDeepSeek(items, attempt + 1);
    }
    throw new Error(`DeepSeek radar-scoring feilet: ${body?.error?.message || `HTTP ${response.status}`}`);
  }
