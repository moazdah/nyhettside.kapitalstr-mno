...
export async function getAdminData() {
  const sql = db();
  await Promise.all([ensureRawItemScoringSchema(sql), ensureArticleWriterSchema(sql)]);
  ...
  const [queue, published, feed, sources, usage, liveOrder, policyRate, rawItems, radarItems] = await Promise.all([
    ...
    sql`
      SELECT r.id, r.source_name, r.source_domain, r.source_kind, r.title, r.summary, r.url, r.image_url,
             ...
             fp.status AS fact_pack_status, fp.version AS fact_pack_version, fp.headline_fact, fp.event_type AS fact_event_type,
      ...
      ORDER BY r.ai_score DESC NULLS LAST, r.published_at DESC NULLS LAST, r.discovered_at DESC
      LIMIT 80
    `,
  ]);
...
export async function getDraftDetail(id) {
  ...
  const [article] = await sql`
    SELECT a.id, a.slug, a.status, a.tittel, a.undertittel, a.brodtekst, a.seksjon,
           a.forfatter, a.ai_score, a.ai_begrunnelse, a.ai_modell, a.tall_validert,
           ...
           ap.primary_source_url, ap.credit_required, ap.generation_model,
