...
export async function getAdminData() {
  const sql = db();
  const [queue, published, feed, sources, usage, liveOrder, policyRate, rawItems] = await Promise.all([
    sql`
  ...
  return { queue, published, feed, sources, usage, liveOrder, policyRate: policyRate[0] || null, rawItems };
}
