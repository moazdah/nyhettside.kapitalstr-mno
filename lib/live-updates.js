...
- En score på 60–69 kan publiseres i live-strømmen dersom den er fersk og konkret; dette er ikke samme terskel som for fullartikkel.
  ...
  for (const item of returned) {
    ...
    const inserted = await sql`
      ...
      SELECT
        ...
        ${source.event_key || null}, ${generatedBy}
      WHERE NOT EXISTS (
...
export async function syncLiveUpdatesForRun(runId, {
  ...
  const candidates = await sql`
    SELECT r.id, r.title, r.summary, r.ai_score, r.ai_section, r.ai_reason,
           r.source_name, r.source_domain, r.url,
...
export async function syncLiveUpdatesFromRecentRadar({
  ...
  const candidates = await sql`
    SELECT r.id, r.title, r.summary, r.ai_score, r.ai_section, r.ai_reason,
           r.source_name, r.source_domain, r.url,
           COALESCE(r.event_key, r.local_event_key) AS event_key,
           r.attention_score, r.local_priority,
    ...
    WHERE r.local_triage_status = 'candidate'
      ...
      AND COALESCE(r.published_at, r.discovered_at) >= NOW() - interval '4 hours'
      AND r.ai_scored_at >= NOW() - interval '3 hours'
      AND NOT EXISTS (
        SELECT 1
        FROM feed f
        WHERE f.radar_item_id = r.id
    ...
    ORDER BY COALESCE(r.published_at, r.discovered_at) DESC,
             r.ai_score DESC,
  ...
  return generateUpdates(sql, candidates, { autoPublish, generatedBy: 'live-pulse-v1' });
}
