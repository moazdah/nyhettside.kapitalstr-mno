...
function parseSourcePublishedAt(html = '', url = '') {
  const text = String(html || '').slice(0, 1_800_000);
  const patterns = [
    /<meta[^>]+(?:property|name|itemprop)=["']article:published_time["'][^>]+content=["']([^"']+)["']/i,
    /<meta[^>]+(?:property|name|itemprop)=["']datePublished["'][^>]+content=["']([^"']+)["']/i,
...
async function verifyLiveSourceFreshness(sql, row) {
  ...
  if (row.live_source_checked_at) {
    return row.live_source_fresh === true;
  }
