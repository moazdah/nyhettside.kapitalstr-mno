...
const FACT_PACK_VERSION = 'fact-pack-v6';
...
function isLikelyCorporatePrimaryUrl(url, item) {
  try {
    ...
    if (!brand || brand.length < 5) return false;
    ...
    return compactIdentityText(item).includes(brand);
  } catch {
    return false;
  }
}
  ...
  try {
    ...
    for (const query of queries) {
      ...
      for (const article of articles) {
        ...
        const knownOfficial = isSpecificPrimaryUrl(articleUrl);
        const corporateOfficial = isLikelyCorporatePrimaryUrl(articleUrl, item);
        if (!knownOfficial && !corporateOfficial) continue;
        ...
        if (knownOfficial) score += 1.0;
        if (corporateOfficial) score += 1.2;
        ...
        if (!existing || score > existing.score) {
          seen.set(articleUrl, {
            ...
            role: 'primary',
            quality: corporateOfficial ? 'corporate_official' : 'official',
            score,
          });
        }
      }
