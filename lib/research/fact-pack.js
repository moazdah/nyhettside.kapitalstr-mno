...
async function supportingSourcesFromCluster(sql, item, mainSource) {
  ...
  // Bound total source time so research can finish within its four-minute lease.
  for (const source of candidates.filter(source => source?.url && source.url !== mainSource?.url).slice(0, 3)) {
