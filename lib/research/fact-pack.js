...
KILDEPRINSIPP:
- En offisiell primærkilde er sterkest, men én etablert, troverdig finans-/nyhetskilde kan være tilstrekkelig grunnlag for et redaksjonelt utkast.
- "trusted_secondary" betyr en etablert redaksjonell aktør som DN, E24, Finansavisen, Reuters, Bloomberg eller FT.
- Flere kilder er nyttig når de finnes, men er IKKE et krav for at en vanlig nyhetssak skal kunne skrives.
- Unike råd, intervjuer, eksklusive opplysninger og vurderinger fra andre medier skal beholde tydelig attribusjon til kilden.
- Hvis hovedkilden er offisiell og en sterk støttekilde har en relevant tilleggsopplysning, kan begge brukes. Hvert faktum skal angi hvilken kilde som støtter det.
...
- unknowns er KUN interne redaksjonsnotater. De skal aldri senere presenteres for leseren som setninger som "kilden bekrefter ikke".
- Hvis en støttekilde dokumenterer en opplysning som hovedkilden ikke omtaler, er opplysningen fortsatt brukbar med naturlig attribusjon til støttekilden.
...
Returner kun gyldig JSON:
{
  ...
  "facts":[{"fact":"konkret faktum","source_name":"navn på kilden","source_role":"primary eller trusted_secondary","attribution_needed":false}],
  "numbers":[{"label":"hva tallet gjelder","value":"nøyaktig verdi","unit":"enhet eller tom streng","context":"kort kontekst","source_name":"navn på kilden"}],
  "entities":["navn"],
  ...
  const user = {
    ...
    main_source: {
      ...
      quality: primary.quality || 'official',
      text: compactSourceText(sourceText, item.title),
    },
    supporting_source: supporting && supporting.url !== primary.url ? {
...
export async function buildFactPackForRadarItem(id) {
  ...
  await sql`
    INSERT INTO fact_packs (
      ...
      source_role, source_quality, source_fetched_at, source_hash, headline_fact, event_type, facts, numbers,
  ...
  await sql`
    UPDATE radar_items
    SET primary_source_status = ${primary.role === 'trusted_secondary' ? 'trusted_secondary' : 'verified'},
  ...
  await sql`
    ...
    WHERE NOT EXISTS (
      ...
      WHERE radar_item_id = ${item.id}
        AND role = ${primary.role === 'trusted_secondary' ? 'trusted_secondary' : 'primary'}
        AND url = ${primary.url}
    )
  `;
