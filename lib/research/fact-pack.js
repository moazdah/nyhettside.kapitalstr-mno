...
const FACT_PACK_VERSION = 'fact-pack-v1';
...
async function fetchPrimarySourceText(url, fallback = '') {
  ...
  try {
    ...
    if (type.includes('application/pdf')) return String(fallback || '').slice(0, 18000);
...
async function resolvePrimarySource(sql, item) {
  if (isTrustedPrimaryUrl(item.url)) {
    return {
  ...
  if (/norges\s+bank/i.test(title) && /(styringsrente|rentevedtak|rentebeslutning|rentemøte)/i.test(title)) {
    return {
      name: 'Norges Bank',
  ...
  if (best && best.score >= 0.5 && best.url && isTrustedPrimaryUrl(best.url)) {
    return {
...
export async function buildFactPackForRadarItem(id) {
  ...
  const primary = await resolvePrimarySource(sql, item);
  if (!primary) return saveNeedsSource(sql, item);
  ...
  const status = pack.canWrite && pack.confidence >= 70 ? 'ready' : 'needs_review';
