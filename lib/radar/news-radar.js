...
const RSS_FEEDS = [
  ...
  {
    name: 'Google News – Fast global finance desks',
    url: 'https://news.google.com/rss/search?q=%28site%3Areuters.com%20OR%20site%3Abloomberg.com%20OR%20site%3Aft.com%20OR%20site%3Acnbc.com%20OR%20site%3Awsj.com%29%20%28markets%20OR%20stocks%20OR%20rates%20OR%20earnings%20OR%20acquisition%20OR%20inflation%20OR%20Fed%20OR%20oil%20OR%20Bitcoin%29%20when%3A2h&hl=en-US&gl=US&ceid=US%3Aen',
    kind: 'fast-news-aggregator',
    maxAgeHours: 2,
  },
  {
    name: 'Google News – Fast Norway finance',
    url: 'https://news.google.com/rss/search?q=%28site%3Ae24.no%20OR%20site%3Adn.no%29%20%28b%C3%B8rs%20OR%20aksjer%20OR%20rente%20OR%20krone%20OR%20selskap%20OR%20oppkj%C3%B8p%20OR%20resultat%29%20when%3A2h&hl=no&gl=NO&ceid=NO%3Ano',
    kind: 'fast-news-aggregator',
    maxAgeHours: 2,
  },
  {
...
async function initializeRadarSchema(sql = db()) {
  await sql`
...
export async function pruneRadarNoise(sql = db()) {
  ...
  for (const id of remove) {
    try {
      ...
      // A case retains its source evidence, including manually chosen items
      // without an AI score. Retention must not break discovery for other items.
...
export async function runNewsRadar({ liveOnly = false } = {}) {
  const sql = db();
  await ensureRadarSchema(sql);
  ...
  const feeds = liveOnly
    ? RSS_FEEDS
        .filter((feed) => /Dagens Næringsliv|E24 – Børs|E24 – Makro|Federal Reserve|ECB|CNBC|Financial Times|WSJ|Fast global finance desks|Fast Norway finance/i.test(feed.name))
        .map((feed) => ({ ...feed, maxAgeHours: Math.min(Number(feed.maxAgeHours || 2), 2) }))
    : RSS_FEEDS;
  ...
  const gdeltQueries = liveOnly
    ? GDELT_QUERIES
        .filter((query) => /makro og sentralbanker|markeder, råvarer og krypto/i.test(query.name))
        .map((query) => ({ ...query, timespan: '2h' }))
    : GDELT_QUERIES;
...
export async function ensureRadarSchema(sql = db()) {
  return schemaOnce(sql, "ensureRadarSchema", () => initializeRadarSchema(sql));
}
