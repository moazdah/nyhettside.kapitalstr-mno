...
export async function ensureLocalTriageSchema(sql = db()) {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_radar_research_retry ON radar_items (research_retry_after, autopilot_run_id)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_radar_draft_retry ON radar_items (draft_retry_after, autopilot_run_id)`;
}
