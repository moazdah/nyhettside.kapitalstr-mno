...
export const RADAR_MODEL_TAG = 'deepseek-v4-pro/radar-v5';
...
const MAJOR_FIGURE = [
  /\b(warren buffett|jerome powell|christine lagarde|jamie dimon|larry fink|ray dalio|howard marks)\b/i,
];
...
const MARKET_MACRO = [
  ...
  /\b(tax|taxes|budget|fiscal policy|regulation|skatt|skatter|budsjett|statsbudsjett|finanspolitikk|regulering)\b/i,
];
...
const BASIC_FINANCE = [
  ...
  /\b(shipping|tanker|dry bulk|offshore|rigg|rigs?|salmon|laks|semiconductor|chip|chips|oil|olje|natural gas|gass)\b/i,
];
...
function localPriority(item) {
  ...
  const figure = count(text, MAJOR_FIGURE);
  ...
  const freshness = Math.max(0, 10 - ageHours * 0.5);

  let score = source + freshness;
  ...
  const financeDeskSource = /Aksjetips|Børs og finans|Dagens Næringsliv|E24|Yahoo Finance|CNBC|WSJ|Financial Times|Reuters|Bloomberg/i.test(String(item.source_name || ''));
  const explicitFinance = corporate > 0
    || macro > 0
    || move > 0
    || (basic > 0 && (financeDeskSource || source >= 14))
    || (figure > 0 && (financeDeskSource || attention > 0))
...
async function initializeLocalTriageSchema(sql = db()) {
  ...
  await sql`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS research_attempts INT NOT NULL DEFAULT 0`;
  await sql`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS research_retry_after TIMESTAMPTZ`;
  await sql`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS research_last_error TEXT`;
  await sql`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS draft_attempts INT NOT NULL DEFAULT 0`;
  await sql`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS draft_retry_after TIMESTAMPTZ`;
  await sql`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS draft_last_error TEXT`;
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_radar_research_retry ON radar_items (research_retry_after)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_radar_draft_retry ON radar_items (draft_retry_after)`;
}
...
export async function prepareRadarCandidates(sql = db(), options = {}) {
  ...
  await sql`
    UPDATE radar_items
    SET local_triage_status = 'expired'
    WHERE local_triage_status = 'candidate'
      AND COALESCE(published_at, discovered_at) < now() - (${WINDOW_HOURS} * interval '1 hour')
  `;
  ...
  for (const cluster of clusters) {
    ...
    for (const item of cluster.items) {
      ...
      updates.push({
        id,
        status,
        priority: id === representativeId ? Math.round(cluster.priority) : Number(item.local_priority || 0),
...
export async function ensureLocalTriageSchema(sql = db()) {
  return schemaOnce(sql, "ensureLocalTriageSchema", () => initializeLocalTriageSchema(sql));
}
