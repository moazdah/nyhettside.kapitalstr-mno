...
function localPriority(item) {
  ...
  const explicitFinance = corporate > 0 || macro > 0 || move > 0 || (company > 0 && /\b(revenue|profit|eps|margin|guidance|shares?|stock|deal|acquisition|earnings|results?)\b/i.test(text));
  return {
...
export async function prepareRadarCandidates(sql = db(), options = {}) {
  ...
  const rows = await sql`
    ...
    FROM radar_items
    WHERE COALESCE(published_at, discovered_at) >= now() - interval '${WINDOW_HOURS} hours'
  ...
  for (const cluster of clusters) {
    ...
    for (const item of cluster.items) {
      ...
      await sql`
        UPDATE radar_items
        SET local_triage_status = ${status},
            local_priority = ${Number(item.local_priority || 0)},
            local_event_key = ${cluster.key},
            local_cluster_size = ${cluster.size},
            local_representative_id = ${representativeId},
  ...
  for (const id of noiseIds) {
    await sql`
      UPDATE radar_items
      SET local_triage_status = 'noise',
          local_priority = 0,
          local_cluster_size = 1,
          local_representative_id = id,
          local_triaged_at = now()
      WHERE id = ${id}
    `;
  }
