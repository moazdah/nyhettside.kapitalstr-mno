...
const FACT_PACK_VERSION = 'fact-pack-v8';
const SCORE_BATCH = 8;
...
export async function runAutopilotStep({ discovery = false, runId = null, mode = 'manual' } = {}) {
  const sql = db();
  await ensureArticleWriterSchema(sql);
  await ensureEditorialSelectionSchema(sql);
  ...
  if (!activeRunId) {
    const run = await createEditorialRun(sql, mode === 'scheduled' ? 'scheduled' : 'manual');
