...
import { syncLiveUpdatesForRun } from '../live-updates';

const FACT_PACK_VERSION = 'fact-pack-v9';
...
export async function getAutopilotState(sql = db(), runId = null) {
  ...
  if (runId) {
    const [researchRows, deferredResearchRows, draftRows, deferredDraftRows, selectedRows, createdRows] = await Promise.all([
      sql`
        SELECT COUNT(*)::int AS count
        FROM radar_items r
    ...
    draftsCreated = Number(createdRows[0]?.count || 0);
    const remainingDraftSlots = Math.max(0, Number(run?.selection_limit || AUTO_SELECTION_LIMIT) - draftsCreated);
    drafting = Math.min(Number(draftRows[0]?.count || 0), remainingDraftSlots);
    deferredDrafting = Math.min(Number(deferredDraftRows[0]?.count || 0), remainingDraftSlots);

    // Once the run has produced its article quota, do not keep researching backup candidates.
  ...
  if (runId && run) {
    ...
    } else if (run.selection_done && drafting > 0) {
      const articleLimit = Number(run.selection_limit || AUTO_SELECTION_LIMIT);
      progressPct = Math.min(99, Math.round(88 + (draftsCreated / Math.max(1, articleLimit)) * 11));
...
async function nextDraftIds(sql, runId) {
  ...
  const remainingSlots = Math.max(
    0,
    Number(run?.selection_limit || AUTO_SELECTION_LIMIT) - Number(created?.count || 0)
  );
  ...
  const rows = await sql`
    ...
    WHERE r.autopilot_run_id = ${Number(runId)}
      ...
      AND (r.draft_retry_after IS NULL OR r.draft_retry_after <= now())
    ORDER BY r.selection_score DESC NULLS LAST,
             fp.confidence DESC NULLS LAST,
             r.ai_score DESC,
             r.attention_score DESC NULLS LAST
    LIMIT ${Math.min(DRAFT_BATCH, remainingSlots)}
  `;
  return rows.map((row) => Number(row.id));
}
...
export async function runAutopilotStep({ discovery = false, runId = null, mode = 'manual', scheduledSlot = null } = {}) {
  ...
  if (before.selectionPending) {
    ...
    if (mode === 'scheduled' && result.selected.length) {
      try {
        const settings = await getEditorialSettings(sql);
        liveUpdates = await syncLiveUpdatesForRun(activeRunId, {
          autoPublish: settings?.autoPublishEnabled === true,
        });
        if (Array.isArray(liveUpdates?.errors)) liveErrors.push(...liveUpdates.errors);
      } catch (error) {
        liveErrors.push(String(error?.message || 'Kunne ikke oppdatere nyhetsstrømmen.').slice(0, 500));
      }
    }
  ...
  // A completed current round must be closed before opportunistic retry work.
  // Otherwise old deferred items can keep today's dashboard stuck on "running".
  if (before.done) {
  ...
  if (recoveryDraftIds.length) {
    ...
    try {
      ...
      if (result?.articleId && mode === 'scheduled') {
        const [sourceRun] = await sql`
          ...
          LIMIT 1
        `;
        const publicationSettings = await getEditorialSettings(sql);
        if (publicationSettings?.autoPublishEnabled === true && sourceRun?.autopilot_run_id) {
          publication = await publishAutopilotArticle(result.articleId, {
            runId: Number(sourceRun.autopilot_run_id),
          });
        }
      }
