...
export async function getEditorialRun(sql, runId) {
  ...
  const [run] = await sql`
    SELECT id, mode, status, selection_limit, selected_count, scoring_total,
           discovery_done, triage_done, selection_done, scheduled_slot, started_at, finished_at
    FROM editorial_runs
...
export async function markTriageDone(sql, runId, stats = {}) {
  ...
  await sql`
    ...
    SET triage_done = true,
        ...
        notes = COALESCE(notes, '{}'::jsonb) || ${JSON.stringify({ local_triage: safeStats })}::jsonb
    WHERE id = ${Number(runId)}
  `;
}
...
function editorialPriority(item) {
  ...
  return (score * 0.50) + (attention * 0.22) + (numbers * 0.14) + freshness + credibility;
}
          ...
          AND (
            ...
            OR EXISTS (
              ...
              WHERE recent_article.radar_item_id = recent_pick.id
                AND recent_article.status IN ('draft', 'live')
            )
          )
      )
